...
# ===================== 顶层：Per-Band GCN → （去掉时频模块）→ (F均值 + T池化) → FC =====================
class STGCN_PB_NoTF_FC(nn.Module):
    """
    消融版本：完全移除时频卷积模块。
    流程:  Per-Band GCN（空间）→ 频段均值融合 → 时间池化（mean）→ 展平节点 → FC
    说明:
      - 不在 T 或 F 维度上做任何卷积；仅做 (F,T) 的平均操作以得到全局时频汇聚特征。
      - 为了接口兼容，self.Ctf 被设置为 Cmid（不再使用独立的 Ctf 通道数）。
    """
    def __init__(self, A_hat, Freq=5, Cb=16, Cmid=32, Ctf=48,   # Ctf 参数被忽略
    ...
    def forward(self, x_BTFE):
        ...
        # 2) 频段均值融合（不做 F 卷积） → [B,T,E,Cmid]
        H_fused = H_stack.mean(dim=2)

        # 3) 时间池化（mean over T；不做 T 卷积） → [B,E,Cmid]
        feat = H_fused.mean(dim=1)

        # 4) 展平节点 → FC
        flat = feat.reshape(B, E * self.Ctf)         # [B, E*Cmid]
