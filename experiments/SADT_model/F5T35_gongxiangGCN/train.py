...
# <--- 修改点: 不再需要导入 compute_stats_crossband --->
from data_loader import EEGDataLoader, EEGDataset
from model import STGCN_PB_TFConv_FC_SharedGCN
...
RESULT_DIR = "result_stgcn_strict_persubject_norm" # <--- 建议修改结果目录名以作区分
CKPT_DIR   = "checkpoints_stgcn_strict_persubject_norm"
...
def main():
    ...
    # 3) 模型
    model = STGCN_PB_TFConv_FC_SharedGCN(A_hat=A_hat, Freq=5, num_classes=2).to(device)
    ...
    for ep in range(1, EPOCHS+1):
        ...
        hist.append({
            'epoch': ep, 'train_loss': tr_loss, 'train_acc': tr_met['acc'], 'train_precision': tr_met['precision'],
    ...
    pd.DataFrame([{
        'best_epoch': best_ep, 'val_best_acc': best_acc,
