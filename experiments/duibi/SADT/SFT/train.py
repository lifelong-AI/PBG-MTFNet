...
RESULT_DIR = "result_sftnet"
CKPT_DIR = "checkpoints_sftnet"
...
def split_indices(n_total, splits=(0.6,0.2,0.2), seed=42):
    ...
    rng.shuffle(idx)
    n_tr = int(n_total*splits[0])
    n_va = int(n_total*splits[1])
...
def eval_epoch(model, loader, loss_fn, device):
    ...
    ps = []
    for batch in loader:
        x = batch['input_hw'].to(device)  # [B,T,F,H,W]
...
def main():
    ...
    # 3) 模型（只用 SFTNet）
    model = SFTNet(input_dim=(8, 5, loader.hw[0], loader.hw[1]), pred_steps=1).to(device)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if va_met['acc'] > best_acc:
            best_acc, best_ep = va_met['acc'], ep
